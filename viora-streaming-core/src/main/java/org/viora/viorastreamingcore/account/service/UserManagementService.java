...
public class UserManagementService implements RegisterUserUseCase, GetUserAccountUseCase {
  ...
  public Account registerUser(RegisterUserRequest request) {
    if (accountRepository.existsByLogin(request.login())) {
      throw new AccountAlreadyExistsException(
          "Account with login " + request.login() + " already exists");
    }
    AccountModel model = new AccountModel(null, request.login(),
  ...
  public Account findAccountByLogin(String login) {
    return accountRepository.findByLogin(login)
        ...
        .orElseThrow(
            () -> new AccountNotFoundException("Account with login " + login + " not found"));
  }
